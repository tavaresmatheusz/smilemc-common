		...
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (sender instanceof ProxiedPlayer) {
					...
					if (member == null) {
						p.disconnect(TextComponent.fromLegacyText("§cDesculpe, ocorreu um erro ao consultar sua conta."));
						return true;
					}

					if (!member.hasGroupPermission(command.groupToUse())) {
						p.sendMessage(TextComponent.fromLegacyText(
								"§cVocê não possui as permissões necessárias para utilizar este comando!"));
						return true;
					}
				}
